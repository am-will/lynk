...
- Host bridge session persistence is harness-specific. OpenClaw history lives in OpenClaw Gateway. Hermes and Codex completed session history can be cached to disk by the bridge, but active in-flight runs are not resumed after a bridge restart and SSE clients do not receive a synthetic terminal event from the restarted process.
