...
On Unix-like hosts Lynk enforces `0700` directories and `0600` sensitive files, writes config/session generations through a synced temporary file and atomic rename, syncs the parent directory, retains a previous backup, and quarantines corrupt primaries before recovery. Concurrent writer instances use isolated destination-sibling temporary files; every completed rename is valid and the last rename wins. Writers remove only their own live temporary file. Startup scavenging examines at most 256 matching leftovers and removes only regular, non-symlink temporary files older than 24 hours, so it cannot delete a fresh foreign writer's in-flight generation. Legacy CWD `state/*-sessions.json` and config-adjacent Devin catalogs are copied, verified, and marked idempotently; the legacy source is retained. Audit files contain allowlisted identifiers/status/metrics only—never raw prompts, message bodies, RPC parameters, attachments, tokens, capabilities, or tool results—and rotate at 2 MiB with a 20 MiB/14-day total retention policy.
