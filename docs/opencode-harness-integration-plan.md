...
- 2026-06-01 Task 8: Added fake OpenCode server/SSE coverage for model and session normalization, workspace directories, prompt streaming, aborts, permission replies, commands, tools, router/protocol contracts, Android config, presentation, workspace grouping, and tool action parsing; wired the provided OpenCode SVG into the Android brand drawable. Files changed: `android/app/src/main/java/dev/androidagent/overlay/ChatPresentationHelpers.kt`, `android/app/src/main/res/drawable/opencode_logo.xml`, `android/app/src/test/java/dev/androidagent/AgentConfigModeTest.kt`, `android/app/src/test/java/dev/androidagent/overlay/ChatPresentationHelpersTest.kt`, `docs/opencode-harness-integration-plan.md`, `opencode-logo.svg`, `pc/src/bridge/OpenClawChatBridge.test.ts`, `pc/src/bridge/OpenClawGatewayEventRouter.test.ts`, `pc/src/bridge/harness/HarnessChatRouter.test.ts`, `pc/src/bridge/opencode/OpenCodeChatClient.test.ts`, `pc/src/protocol/contracts.test.ts`. Tests: `cd pc && npm run check`; `cd pc && npm test`; `cd android && ./gradlew :app:testDebugUnitTest`. Commit: `425465e`.
- 2026-06-01 Task 9: Completed final verification for the OpenCode harness integration, including PC typecheck/tests, Android unit tests, and debug APK assembly after Android source/resource changes. Files changed: `docs/opencode-harness-integration-plan.md`. Tests: `cd pc && npm run check && npm test`; `cd android && ./gradlew :app:testDebugUnitTest`; `cd android && ./gradlew :app:assembleDebug`. Commit: pending.
