...
- Step 4: Extracted OpenCode session listing, merging, empty-session filtering, and workspace summaries into `pc/src/bridge/opencode/OpenCodeSessionCatalog.ts`; `OpenCodeChatClient.ts` is now 719 lines. Verified with `cd pc && npm run check` and OpenCode-focused tests.
- Step 5: Extracted OpenCode run orchestration into `OpenCodeRunDriver.ts`, event mapping into `OpenCodeEventNormalizer.ts`, and removed model/agent from the server `createSession` contract. `OpenCodeChatClient.ts` is now 262 lines. Verified with `cd pc && npm run check` and OpenCode-focused tests, including stream-error cleanup.
- Step 6: Split OpenClaw bridge harness/readiness/session tests into `OpenClawChatBridgeHarness.test.ts` with shared fixtures in `OpenClawChatBridge.testSupport.ts`; `OpenClawChatBridge.test.ts` is now 752 lines. Verified with `cd pc && npm run check` and the affected bridge test files.
- Step 7: Extracted protocol phone command helpers, OpenClaw readiness helpers, and chat message builders so `messages.ts` is 984 lines and `OpenClawChatBridge.ts` is 997 lines. Verified with `cd pc && npm run check && npm test` with 187 passing tests.
