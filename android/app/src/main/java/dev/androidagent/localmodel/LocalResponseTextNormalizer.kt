...
object LocalResponseTextNormalizer {
    fun normalize(text: String): String {
        return visibleStreamingText(text).trim()
            ...
            .replace(Regex("""(?m)(\d+)\.\*\s*\n+\s*\*\s*([A-Z][^:\n]{2,80}:)\*\*"""), "\n\n$1. **$2**")
            .replace(Regex("""(?m)(\d+)\.\*\s*\n+\s*([A-Z][^:\n]{2,80}:)\*\*"""), "\n\n$1. **$2**")
            .replace(Regex("""(?m)(\d+)\.\*\*([A-Z][^:\n]{2,80}:)\*\*"""), "\n\n$1. **$2**")
            .replace(Regex("""(?m)(\d+)\.([A-Z][^:\n]{2,80}:)"""), "\n\n$1. **$2**")
            .replace(Regex("""(?m)^(#{1,6}\s+\d+\.\s+[A-Z][A-Za-z ]+?)(?=(In|When|Because|While|The|This|It)\b)"""), "$1\n\n")
            .replace(Regex("""(?m)^(#{1,6}\s+[A-Z][A-Za-z0-9 "'()\-]+?)(?=(In|When|Because|While|The|This|It)\b)"""), "$1\n\n")
            .replace(Regex("""(?m)^([A-Z][A-Za-z0-9 "'()\-]+?)(?=(In|When|Because|While|The|This|It)\b)"""), "$1\n\n")
